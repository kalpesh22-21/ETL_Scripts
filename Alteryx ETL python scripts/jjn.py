...
def extract_zip(file_dir, output_dir, Output_name , name='Alteryx Inputs'):
    # Extract_files
    ...
    for File in list_files:
        if File.endswith('.ZIP') or File.endswith('.zip'):
            with ZipFile(file_dir + '\\' + File, 'r') as z:
                z.extractall(output_dir + '/' + name+'/' + File.rstrip('.zip'))
            print('Saved File : ' + str(File))
            z.close()
    # Saving all excels in one file
    single_excel(file_dir, output_dir,Output_name)
    return ()

def single_excel(file_dir, output_dir, Output,name='Alteryx Inputs'):
...
lastWeekDateTime = dt.datetime.now() - dt.timedelta(days=20)
...
Myfolder = mydesktop + 'Outlook_Export\\'
                ...
                if stat == True:
                    ...
                    for att in message.Attachments:
                        if att.FileName.endswith('.msg'):
                            ...
                            extract_zip(Myfolder_1, Myfolder_1,accepted_file_formats[idx] + ' ' + file_month)
